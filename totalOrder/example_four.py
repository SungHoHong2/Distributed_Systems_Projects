    ...
    def loop(self):
        ...
        if (self.flag < 20):
            if (int(procNo) == 0):
                ...
                # process 2 sending message
                send = Message((procNo), str(int(procNo) + 4), (self.clock), False, False, int(procNo), int(self.clock))
                self.sendUpdate(send)
...
if __name__ == '__main__':
    ...
    # the third process creates two clients
        ...
        print "Connecting to host " + address + " port " + str(port2)
        reactor.connectTCP(address, port2, client1)
        print "Connecting to host " + address + " port " + str(port3)
        reactor.connectTCP(address, port3, client2)
